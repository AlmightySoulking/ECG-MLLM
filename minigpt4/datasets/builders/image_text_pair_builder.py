...
@registry.register_builder("ecg_dataset")
class ECGDatasetBuilder(BaseDatasetBuilder):
    train_dataset_cls = ECGDataset
    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/ecg/default.yaml",
    }
...
class AllRefCOCOBuilder(BaseDatasetBuilder):
    ...
    def build_datasets(self):
        ...
        if not os.path.exists(vis_root):
            warnings.warn("image path {} does not exist.".format(vis_root))
