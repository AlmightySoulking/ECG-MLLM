...
class MiniGPTBase(BaseModel):
    ...
    def __init__(
        ...
        self.visual_encoder, self.ln_vision = self.init_ecg_encoder(
            ecg_model, patch_size=patch_size, seq_len=seq_len, freeze=freeze_ecg
        )
    ...
    def get_bos_token_id(self):
        return (
            ...
            if self.llama_tokenizer.bos_token_id is not None
            else self.get_eos_token_id()
        )
    ...
    def get_eos_token_id(self):
        ...
        if eos_token_id is not None:
            return eos_token_id
        return self.llama_tokenizer.pad_token_id

    def get_stop_words_ids(self, stop_text="###"):
        stop_ids = self.llama_tokenizer.encode(stop_text, add_special_tokens=False)
        if stop_ids:
            return stop_ids

        eos_token_id = self.get_eos_token_id()
        return [eos_token_id] if eos_token_id is not None else None
    ...
    def forward(self, samples,):
        ...
        if bos_token_id is None:
            raise ValueError("The loaded tokenizer is missing both bos_token_id and eos_token_id.")
        bos = torch.ones_like(part_targets[:, :1]) * bos_token_id
